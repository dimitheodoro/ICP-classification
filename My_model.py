...
def MultipleInputsModel(input_shape, sex_label_shape, age_label_shape,GCS_label_shape, sex_num_classes=2, age_num_classes=3,GCS_num_classes=3):
    ...
    x = Conv2D(64, (3, 3), activation='relu',kernel_initializer=initializer)(merge)
...
def MultipleInputsModel_TURBO(input_shape, sex_label_shape, age_label_shape,GCS_label_shape, sex_num_classes=2, age_num_classes=3,GCS_num_classes=3):

    output_bias = tf.keras.initializers.Constant(-0.48121182505960345)
    initializer = HeNormal(seed=42)
    # Functional API
    sex_label =Input(sex_label_shape)  #shape=(1,)
    sex = Embedding (sex_num_classes,50)(sex_label)
